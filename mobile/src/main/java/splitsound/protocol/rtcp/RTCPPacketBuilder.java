...
import splitsound.protocol.*;

/**
 * Created by J. McKernan on 2/25/2018.
 * Update by J. McKernan on 2/28/2018.
 */
...
public class RTCPPacketBuilder
{
    ...
    public int setMaximumPacketSize(long maxPackSize)
    {
        if (!initialized)
            return RTPError.ERR_RTP_RTCPPACKETBUILDER_NOTINIT;
        if (maxPackSize < RTPConstants.RTP_MINPACKETSIZE)
            return RTPError.ERR_RTP_RTCPPACKETBUILDER_ILLEGALMAXPACKSIZE;
    ...
    public int buildNextPacket(RTCPCompoundPacket pack) // TODO: was a **
    {
        ...
        if (!processingSDES)
        {
            ...
            if (!flags.full)
            {
                ...
                // Track processedAll and itemCount with FlagTracker
                if ((status = fillInSDES(rtcpCompPack, flags)) < 0)
                {
                    return status;
                }

                if (flags.processedAll)
                {
                    processingSDES = false;
                    clearAllSDESFlags();
                    if (!flags.full && flags.skipped > 0)
                    {
                        // if the packet isn't full and we skipped some
                        // sources that we already got in a previous packet,
                        // we can add some of them now

                        flags.atEndOfList = false;

                        if ((status = fillInReportBlocks(rtcpCompPack, curTime, flags)) < 0)
                        {
                            return status;
                        }
                    }
        ...
        {
            // FlagTracker keeps track of processedAll, itemCount, and full
            ...
            if (flags.itemCount == 0) // Big problem: packet size is too small to let any progress happen
            {
                return RTPErrors.ERR_RTP_RTCPPACKETBUILDER_PACKETFILLEDTOOSOON;
            }
            ...
            if (flags.processedAll)
            {
                ...
                if (!flags.full)
                {
                    ...
                    if ((status = fillInReportBlocks(rtcpCompPack, curTime, sources.getTotalCout(), flags)) < 0)
                    {
                        return status;
                    }
                    if (flags.atEndOfList) // filled in all possible sources
                        clearAllSourceFlags();
                }
            }
        }
    ...
    private void clearAllSourceFlags()
    {
        if (sources.gotoFirstSource())
        {
            do
            {
                RTPSourceData srcData = sources.getCurrentSourceInfo();
                srcData.setProcessedInRTCP(false);
    ...
    private int fillInSDES(RTCPCompoundPacketBuilder rtcpCompPack, FlagTracker flags)
    {
        int status;
        byte[] data;
        long dataLen;
        ...
        if (doName)
        {
            ...
            {
                data = ownSDESInfo.getName();
                if ((status = rtcpCompPack.addSDESNormalItem(RTCPSDESPacket.NAME, data)) < 0)
                {
                    if (status == RTPError.ERR_RTP_RTCPCOMPPACKBUILDER_NOTENOUGHBYTESLEFT)
                    {
                        flags.full = true;
        ...
        if (doEmail)
        {
            ...
            {
                data = ownSDESInfo.getEmail();
                if ((status = rtcpCompPack.addSDESNormalItem(RTCPSDESPacket.EMAIL, data)) < 0)
                {
                    if (status == RTPError.ERR_RTP_RTCPCOMPPACKBUILDER_NOTENOUGHBYTESLEFT)
                    {
                        flags.full = true;
        ...
        if (doLoc)
        {
            ...
            {
                data = ownSDESInfo.getLocation();
                if ((status = rtcpCompPack.addSDESNormalItem(RTCPSDESPacket.LOC, data)) < 0)
                {
                    if (status == RTPError.ERR_RTP_RTCPCOMPPACKBUILDER_NOTENOUGHBYTESLEFT)
                    {
                        flags.full = true;
        ...
        if (doPhone)
        {
            ...
            {
                data = ownSDESInfo.getPhone();
                if ((status = rtcpCompPack.addSDESNormalItem(RTCPSDESPacket.PHONE, data)) < 0)
                {
                    if (status == RTPError.ERR_RTP_RTCPCOMPPACKBUILDER_NOTENOUGHBYTESLEFT)
                    {
                        flags.full = true;
        ...
        if (doTool)
        {
            ...
            {
                data = ownSDESInfo.getTool();
                if ((status = rtcpCompPack.addSDESNormalItem(RTCPSDESPacket.TOOL, data)) < 0)
                {
                    if (status == RTPError.ERR_RTP_RTCPCOMPPACKBUILDER_NOTENOUGHBYTESLEFT)
                    {
                        flags.full = true;
        ...
        if (doNote)
        {
            ...
            {
                data = ownSDESInfo.getNote();
                if ((status = rtcpCompPack.addSDESNormalItem(RTCPSDESPacket.NOTE, data)) < 0)
                {
                    if (status == RTPError.ERR_RTP_RTCPCOMPPACKBUILDER_NOTENOUGHBYTESLEFT)
                    {
                        flags.full = true;
    ...
    public int buildBYEPacket(RTCPCompoundPacket pack, String reason, boolean useSRIfPossible)
    {
        ...
        rtcpCompPack = RTCPCompoundPacketBuilder();
        ...
        if (useSR)
        {
            RTPTime curTime = RTPTime.currenTime();
            RTPTime rtpPackTime = rtpPacketBuilder.getPacketTime();
            int rtpPacketTimestamp = rtpPacketBuilder.getPacketTimestamp();
            int packCount = rtpPacketBuilder.getPacketCount();
            int octectCount = rtpPacketBuilder.getPayloadOctetCount();
        ...
        if ((status = rtcpCompPack.addByePacket(ssrcs, 1, reason)) < 0)
        {
            if (status == RTPError.ERR_RTP_RTCPCOMPPACKBUILDER_NOTENOUGHBYTESLEFT)
                return RTPError.ERR_RTP_RTCPPACKETBUILDER_PACKETFILLEDTOOSOON;
			return status;
     ...
     *  allowed. Otherwise, a receiver report is used.
     */
    public int buildBYEPacket(RTCPCompoundPacket pack, String reason)
    {
